...
app.include_router(
    procurement_router,
    prefix=f"{settings.API_STR}/procurement",
...
def startup_event():
    ...
    db = SessionLocal()
    try:
        from models import Role, User, Project, WorkPackage, Material, MaterialRequest, DailyReport, MaterialUsed, SitePhoto, IssueReport, WorkerPayslip, FeedbackMessage, ProjectDocument, Contractor, Worker, SiteEngineer, MaterialCategory, Inventory, MaterialAllocation, StockMovement, ProcurementCategory
        ...
        # Module 7 default categories must always exist, including existing databases.
        if db.query(ProcurementCategory).count() == 0:
            db.add_all([
                ProcurementCategory(id="PC-RAW", name="Raw Materials", description="Construction raw materials"),
                ProcurementCategory(id="PC-EQP", name="Equipment", description="Tools and equipment"),
                ProcurementCategory(id="PC-MCH", name="Machinery", description="Construction machinery"),
                ProcurementCategory(id="PC-SAFE", name="Safety Equipment", description="Safety and protective equipment"),
                ProcurementCategory(id="PC-OFF", name="Office Supplies", description="Office and administrative supplies")
            ])
            db.commit()
            print("Module 7 procurement categories initialized.")

...
from models import (
    ...
    PurchaseOrderItem, GoodsReceipt, GoodsReceiptItem, ProcurementInvoice, ProcurementPayment,
...
class ProcurementService:
    ...
    def get_vendors(self, db: Session, category: str = None, status: str = None, search: str = None,
        ...
        return [self._serialize_vendor(v) for v in vendors], total
    ...
    def get_categories(self, db: Session):
        return [{"id": c.id, "name": c.name, "description": c.description,
                ...
                for c in db.query(ProcurementCategory).order_by(ProcurementCategory.name).all()]
    ...
    def get_requests(self, db: Session, project_id: str = None, status: str = None,
        ...
        return [self._serialize_request(r) for r in reqs], total
    ...
    def get_purchase_orders(self, db: Session, project_id: str = None, vendor_id: str = None,
        ...
        return [self._serialize_po(po, include_items=True) for po in pos], total
    ...
    def create_purchase_order(self, db: Session, user_id: str, data: dict) -> PurchaseOrder:
        ...
        if not project:
            raise HTTPException(status_code=404, detail="Project not found")
        # A PO must have an active vendor and at least one item.
        if not data.get("vendor_id"):
            raise HTTPException(status_code=400, detail="Vendor is required to create a purchase order")
        vendor = db.query(Vendor).filter(Vendor.id == data["vendor_id"]).first()
        if not vendor:
            raise HTTPException(status_code=404, detail="Vendor not found")
        if vendor.status != "Active":
            raise HTTPException(status_code=400, detail="Selected vendor is not active")
        if not data.get("items"):
            raise HTTPException(status_code=400, detail="At least one purchase order item is required")
        # Every Module 7 purchase order follows an approved procurement request.
        if not data.get("procurement_request_id"):
            raise HTTPException(status_code=400, detail="Approved procurement request is required before creating a purchase order")
        # A project-specific procurement request must be approved before ordering.
        if data.get("procurement_request_id"):
            ...
            if req.project_id != data["project_id"]:
                raise HTTPException(status_code=400, detail="Procurement request belongs to a different project")
            if req.status != "Approved":
                raise HTTPException(status_code=400, detail="Purchase Order can only be created from an approved procurement request")
        # Keep the Module 5 material link from request -> PO. This guarantees a
        # Goods Receipt can update the correct Material Inventory record.
        linked_material_id = req.material_id if data.get("procurement_request_id") else None
        if linked_material_id:
            for item_data in data.get("items") or []:
                if not item_data.get("material_id"):
                    item_data["material_id"] = linked_material_id
                elif item_data.get("material_id") != linked_material_id:
                    raise HTTPException(status_code=400, detail="PO item material must match the linked procurement request material")
        ...
        # Mark linked procurement request as Processing once the official PO is issued.
        if data.get("procurement_request_id"):
            req = db.query(ProcurementRequest).filter(ProcurementRequest.id == data["procurement_request_id"]).first()
            if req:
    ...
    def receive_goods(self, db: Session, user_id: str, data: dict) -> GoodsReceipt:
        ...
        if data.get("project_id") != po.project_id:
            raise HTTPException(status_code=400, detail="Goods receipt project must match the purchase order project")
        if not data.get("items"):
            raise HTTPException(status_code=400, detail="At least one goods receipt item is required")

        received_date = dt.datetime.utcnow()
        if data.get("received_date"):
            received_date = dt.datetime.strptime(data["received_date"], "%Y-%m-%d")

        # Validate every submitted item before writing anything. This prevents over-receiving.
        po_items = {i.id: i for i in db.query(PurchaseOrderItem).filter(PurchaseOrderItem.purchase_order_id == po.id).all()}
        seen = set()
        any_received = False
        for item_data in data["items"]:
            po_item_id = item_data.get("po_item_id")
            if not po_item_id or po_item_id not in po_items:
                raise HTTPException(status_code=400, detail="Every receipt item must belong to the selected purchase order")
            if po_item_id in seen:
                raise HTTPException(status_code=400, detail="A PO item can appear only once in one goods receipt")
            seen.add(po_item_id)
            poi = po_items[po_item_id]
            qty = float(item_data.get("received_quantity", 0))
            if qty < 0:
                raise HTTPException(status_code=400, detail="Received quantity cannot be negative")
            remaining = max(0.0, float(poi.quantity) - float(poi.received_quantity or 0))
            if qty > remaining:
                raise HTTPException(status_code=400, detail=f"Received quantity exceeds remaining quantity for {poi.description}")
            if item_data.get("unit") != poi.unit:
                raise HTTPException(status_code=400, detail=f"Unit does not match PO item {poi.description}")
            if item_data.get("material_id") and poi.material_id and item_data.get("material_id") != poi.material_id:
                raise HTTPException(status_code=400, detail=f"Material does not match PO item {poi.description}")
            any_received = any_received or qty > 0

        if not any_received:
            raise HTTPException(status_code=400, detail="At least one item must have a received quantity greater than zero")

        gr = GoodsReceipt(
            id=f"GR-{str(uuid.uuid4())[:8].upper()}",
            purchase_order_id=po.id,
        ...
        for item_data in data["items"]:
            poi = po_items[item_data["po_item_id"]]
            qty = float(item_data.get("received_quantity", 0))
            db.add(GoodsReceiptItem(
                id=f"GRI-{str(uuid.uuid4())[:8].upper()}",
            ...
            poi.received_quantity = float(poi.received_quantity or 0) + qty
            if poi.material_id and qty > 0:
                self._update_inventory_on_receipt(db, poi.material_id, qty, user_id, gr.id, po.project_id)

        refreshed_items = list(po_items.values())
        fully_received = all(float(i.received_quantity or 0) >= float(i.quantity) for i in refreshed_items)
    ...
    def _update_inventory_on_receipt(self, db: Session, material_id: str, received_qty: float,
        ...
        """Update Module 5 inventory and create a stock movement for the exact receipt quantity."""
        import uuid
        mat = db.query(Material).filter(Material.id == material_id).first()
        if not mat:
            raise HTTPException(status_code=404, detail=f"Linked material {material_id} not found for inventory update")
        inv = db.query(Inventory).filter(Inventory.material_id == material_id).first()
        if not inv:
            inv = Inventory(id=f"INV-{str(uuid.uuid4())[:8].upper()}", material_id=material_id,
                            total_stock=0.0, available_stock=0.0, allocated_stock=0.0, consumed_stock=0.0)
            db.add(inv); db.flush()
        prev_qty = float(inv.available_stock or 0)
        inv.total_stock = float(inv.total_stock or 0) + received_qty
        inv.available_stock = prev_qty + received_qty
        mat.in_stock = inv.available_stock
        db.add(StockMovement(
            id=f"SM-{str(uuid.uuid4())[:8].upper()}", material_id=material_id, project_id=project_id,
            movement_type="Received", quantity=received_qty, date=__import__("datetime").datetime.utcnow(),
            previous_quantity=prev_qty, new_quantity=inv.available_stock, performed_by_id=user_id,
            reference_id=reference_id, remarks=f"Received via Procurement - Goods Receipt {reference_id}"
        ))

    def get_receipts(self, db: Session, po_id: str):
        receipts = db.query(GoodsReceipt).filter(GoodsReceipt.purchase_order_id == po_id).order_by(GoodsReceipt.received_date.desc()).all()
        return [self._serialize_receipt(gr) for gr in receipts]

    # ---- Procurement Payments ----
    def _refresh_invoice_payment_status(self, db: Session, inv: ProcurementInvoice):
        import datetime as dt
        total_paid = sum(float(p.amount or 0) for p in inv.payments)
        inv.paid_amount = total_paid
        if total_paid >= float(inv.invoice_amount):
            inv.payment_status = "Paid"
        elif total_paid > 0:
            inv.payment_status = "Partially Paid"
        elif inv.due_date and inv.due_date < dt.datetime.utcnow():
            inv.payment_status = "Overdue"
        else:
            inv.payment_status = "Pending"

    def record_payment(self, db: Session, user_id: str, invoice_id: str, data: dict):
        import uuid, datetime as dt
        inv = self.get_invoice_by_id(db, invoice_id)
        if inv.invoice_status != "Verified":
            raise HTTPException(status_code=400, detail="Verify the invoice before recording payment")
        amount = float(data.get("amount") or 0)
        if amount <= 0:
            raise HTTPException(status_code=400, detail="Payment amount must be greater than zero")
        already_paid = sum(float(p.amount or 0) for p in inv.payments)
        remaining = float(inv.invoice_amount) - already_paid
        ...
        try:
            payment_date = dt.datetime.strptime(data["payment_date"], "%Y-%m-%d")
        except Exception:
            raise HTTPException(status_code=400, detail="Payment date must be YYYY-MM-DD")
        payment = ProcurementPayment(
            id=f"PAY-{str(uuid.uuid4())[:8].upper()}", invoice_id=inv.id, amount=amount,
            payment_date=payment_date, payment_method=method,
            reference_number=data.get("reference_number") or None, remarks=data.get("remarks") or None,
            recorded_by_id=user_id
        )
        db.add(payment); db.flush(); db.refresh(inv)
        self._refresh_invoice_payment_status(db, inv)
        db.commit(); db.refresh(payment); db.refresh(inv)
        return self._serialize_payment(payment), self._serialize_invoice(inv)

    def get_invoice_payments(self, db: Session, invoice_id: str):
        self.get_invoice_by_id(db, invoice_id)
        rows = db.query(ProcurementPayment).filter(ProcurementPayment.invoice_id == invoice_id).order_by(ProcurementPayment.payment_date.desc(), ProcurementPayment.created_at.desc()).all()
        return [self._serialize_payment(p) for p in rows]

    def _serialize_payment(self, p: ProcurementPayment):
        return {
            "id": p.id, "invoice_id": p.invoice_id, "amount": float(p.amount or 0),
            "payment_date": p.payment_date.isoformat().split("T")[0] if p.payment_date else None,
            "payment_method": p.payment_method, "reference_number": p.reference_number,
            "remarks": p.remarks, "recorded_by_name": p.recorded_by.name if p.recorded_by else None,
    ...
    def get_invoices(self, db: Session, project_id: str = None, vendor_id: str = None,
        ...
        for inv in invoices:
            if inv.due_date and inv.due_date < now and inv.payment_status not in ("Paid",):
                inv.payment_status = "Overdue"
        db.commit()
        return [self._serialize_invoice(inv) for inv in invoices], total
    ...
    def create_invoice(self, db: Session, user_id: str, data: dict) -> ProcurementInvoice:
        ...
        if data.get("purchase_order_id"):
            ...
            if po.status not in ("Partially Received", "Received"):
                raise HTTPException(status_code=400, detail="Invoice can be created only after goods have been received")
            if data.get("project_id") != po.project_id:
                raise HTTPException(status_code=400, detail="Invoice project does not match the purchase order")
            if data.get("vendor_id") != po.vendor_id:
                raise HTTPException(status_code=400, detail="Invoice vendor must match the purchase order vendor")
    ...
    def update_invoice(self, db: Session, inv_id: str, data: dict) -> ProcurementInvoice:
        ...
        # Payment totals and payment status are transaction-derived.
        # Direct paid_amount/payment_status edits would break payment history.
        if data.get("paid_amount") is not None or data.get("payment_status") is not None:
            raise HTTPException(status_code=400, detail="Use the Record Payment endpoint; invoice payment totals are calculated from payment history")
    ...
    def _serialize_vendor(self, v: Vendor) -> dict:
        return {
            "id": v.id, "name": v.name, "contact_person": v.contact_person,
            "contact_number": v.contact_number, "email": v.email, "address": v.address,
            "category": v.category, "products_services": v.products_services, "status": v.status,
            "created_at": v.created_at.isoformat() if v.created_at else None,
            "updated_at": v.updated_at.isoformat() if v.updated_at else None
        }

    def _serialize_request(self, r: ProcurementRequest) -> dict:
        return {
            "id": r.id, "project_id": r.project_id, "project_name": r.project.name if r.project else "",
            ...
            "item_name": r.item_name, "quantity": float(r.quantity or 0), "unit": r.unit,
            ...
            "status": r.status, "remarks": r.remarks, "rejection_reason": r.rejection_reason,
            "available_quantity": r.available_quantity, "shortage_quantity": r.shortage_quantity,
            "material_id": r.material_id, "resource_id": r.resource_id,
            "created_at": r.created_at.isoformat() if r.created_at else None
        }

    def _serialize_po(self, p: PurchaseOrder, include_items: bool = True) -> dict:
        data = {
            "id": p.id, "vendor_id": p.vendor_id, "vendor_name": p.vendor.name if p.vendor else None,
            "project_id": p.project_id, "project_name": p.project.name if p.project else "",
            "procurement_request_id": p.procurement_request_id,
            "created_by_id": p.created_by_id, "created_by_name": p.created_by.name if p.created_by else "",
            ...
            "subtotal": float(p.subtotal or 0), "tax_amount": float(p.tax_amount or 0),
            "additional_charges": float(p.additional_charges or 0), "total_amount": float(p.total_amount or 0),
            "status": p.status, "notes": p.notes,
            "created_at": p.created_at.isoformat() if p.created_at else None
        }
        if include_items:
            data["items"] = [{
                "id": i.id, "purchase_order_id": i.purchase_order_id, "material_id": i.material_id,
                "material_name": i.material.name if i.material else None, "resource_id": i.resource_id,
                "description": i.description, "quantity": float(i.quantity or 0), "unit": i.unit,
                "unit_price": float(i.unit_price or 0), "tax_percent": float(i.tax_percent or 0),
                "line_total": float(i.line_total or 0), "received_quantity": float(i.received_quantity or 0)
            } for i in p.items]
        return data

    def _serialize_receipt(self, gr: GoodsReceipt) -> dict:
        return {
            "id": gr.id, "purchase_order_id": gr.purchase_order_id,
            "vendor_id": gr.vendor_id, "vendor_name": gr.vendor.name if gr.vendor else None,
            "project_id": gr.project_id, "project_name": gr.project.name if gr.project else "",
            ...
            "remarks": gr.remarks, "delivery_note_number": gr.delivery_note_number,
            "created_at": gr.created_at.isoformat() if gr.created_at else None,
            "receipt_items": [{
                "id": i.id, "po_item_id": i.po_item_id, "material_id": i.material_id,
                "material_name": i.material.name if i.material else None, "description": i.description,
                "ordered_quantity": float(i.ordered_quantity or 0), "received_quantity": float(i.received_quantity or 0),
                "unit": i.unit
            } for i in gr.receipt_items]
        }

    def _serialize_invoice(self, inv: ProcurementInvoice) -> dict:
        return {
            "id": inv.id, "invoice_number": inv.invoice_number, "vendor_id": inv.vendor_id,
            "vendor_name": inv.vendor.name if inv.vendor else None, "purchase_order_id": inv.purchase_order_id,
            "project_id": inv.project_id, "project_name": inv.project.name if inv.project else "",
            "created_by_id": inv.created_by_id, "created_by_name": inv.created_by.name if inv.created_by else "",
            "invoice_date": inv.invoice_date.isoformat() if inv.invoice_date else None,
            "due_date": inv.due_date.isoformat() if inv.due_date else None,
            "invoice_amount": float(inv.invoice_amount or 0), "paid_amount": float(inv.paid_amount or 0),
            "payment_status": inv.payment_status, "invoice_status": inv.invoice_status, "remarks": inv.remarks,
            "is_overdue": bool(inv.due_date and inv.due_date < datetime.datetime.utcnow() and inv.payment_status != "Paid"),

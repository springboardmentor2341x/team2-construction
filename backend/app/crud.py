...
def create_project_schedule(
    ...
    notification_service.notify_project_users(
        db=db,
        project_id=schedule.project_id,
        title="New Task Assigned",
        message=f"New task '{schedule.task_name}' assigned to project schedule.",
        type="TASK_ASSIGNMENT",
        related_entity_type="task",
        related_entity_id=db_schedule.id
    )
...
def update_milestone_progress(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Milestone '{milestone.title}' progress updated to {progress_percentage}% ({status}).",
        type="PROJECT_UPDATE",
...
def create_site_engineer_assignment(
    ...
    notification_service.notify_project_users(
        ...
        title="Site Engineer Assigned",
        message=f"Engineer {assignment.engineer_name} has been assigned to project.",
        type="TASK_ASSIGNMENT",
        related_entity_type="site_engineer_assignment",
        related_entity_id=db_assignment.id
    )
...
def create_contractor_assignment(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Contractor {assignment.contractor_name} has been assigned to project.",
        type="TASK_ASSIGNMENT",
        related_entity_type="contractor_assignment",
        related_entity_id=db_assignment.id
    )
...
def update_project_status(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Project status has been updated to '{status}'.",
        type="PROJECT_UPDATE",
...
def create_material_request(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Material request created for project.",
        type="PROCUREMENT",
        related_entity_type="material_request",
        related_entity_id=db_request.id
    )
...
def create_progress_update(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Progress update recorded for '{progress_update.activity_name}'.",
        type="PROJECT_UPDATE",
        related_entity_type="progress_update",
        related_entity_id=db_progress.id
    )
...
def create_attendance(
    ...
    status_val = str(attendance.status or "")
    remarks_val = str(attendance.remarks or "")
    if status_val.lower() in ["absent", "late", "half day", "leave"] or len(remarks_val) > 0:
        pid = db_attendance.project_id if db_attendance.project_id is not None else 1
        notification_service.notify_project_users(
            db=db,
            project_id=pid,
            title="Attendance Issue Alert",
            message=f"Attendance alert: Worker #{db_attendance.worker_id} recorded as {db_attendance.status}.",
            type="ATTENDANCE",
            related_entity_type="attendance",
            related_entity_id=db_attendance.id,
            priority="High",
            target_roles=["Site Engineer", "Project Manager", "Super Admin"]
        )
...
def create_worker_assignment(
    ...
    notification_service.notify_project_users(
        db=db,
        project_id=assignment.project_id,
        title="Worker Assignment Created",
        message=f"Worker assignment created for activity '{assignment.work_activity}'.",
        type="TASK_ASSIGNMENT",
...
def create_procurement_request(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Procurement request created for '{request.item_name}' requiring approval.",
        type="PROCUREMENT",
        related_entity_type="procurement_request",
        related_entity_id=db_request.id,
        target_roles=["Super Admin", "Admin", "Project Manager"]
    )
...
def update_procurement_request_status(
    ...
    notification_service.notify_project_users(
        db=db,
        project_id=request.project_id,
        title=f"Procurement Request {status}",
        message=f"Procurement request for '{request.item_name}' status changed to {status}.",
        type="PROCUREMENT",
        related_entity_type="procurement_request",
...
def create_purchase_order(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Purchase order issued for project {purchase_order.project_id}.",
        type="PROCUREMENT",
        related_entity_type="purchase_order",
        related_entity_id=db_purchase_order.id
    )
...
def create_invoice(
    ...
    notification_service.notify_project_users(
        ...
        message=f"Invoice #{invoice.invoice_number} received.",
        type="PROCUREMENT",
